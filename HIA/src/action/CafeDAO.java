...
public class CafeDAO {
	...
	public ArrayList getCafeList(int page, int limit, String coffee_name, String search) {
		...
		try {
			String sql = "select a.cafe_num, a.cafe_name, a.cafe_file, a.cafe_location, b.price, b.coffee_num from cafe a, coffee b where a.cafe_num = b.cafe_num and b.coffee_name = ? and a.cafe_name like ? limit ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, coffee_name);
			pstmt.setString(2, "%"+search+"%");
...
//=====================================================================================================================
	...
	public boolean insertCart(int[] item, int[] count, String id) {
		...
		for(int i=0; i<item.length; i++) {
			...
			try {
//				sql = "select coffee_num from cart where pay_num=0 and coffee_num=?";
//				pstmt = con.prepareStatement(sql);
...
//	public int[] insertPayment(String id, Date date, int cost) {
...
//	}
	public int[] insertPayment(String id, int cost) {
		...
		int[] pay_info = new int[2];
		pay_info[0] = num;
		sql = "insert into payment values(?,now(),now(),?,?)";
		int insertCount = 0;
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
